...
class EditGltf4Entity {
  ...
  _gltf: any;
  _handler: any;
  _defaultWidth: any; //默认指示线的宽度
  _currentPick: any;
  _dStep: any;
  _rStep: any;
  _params: any;
  _coordArrows: any; //平移指示器
  _coordCircle: any; //旋转指示器
  constructor(viewer: any, gltf: any, d: any, r: any) {
  ...
  returnGltfRotation(gltf: any) {
    ...
    var hpr = Cesium.Transforms.fixedFrameToHeadingPitchRoll(mtx4, this._viewer.scene.globe.ellipsoid);
    return hpr;
  }
  returnGltfCentor(gltf: any) {
    let gltfCenter = gltf.position.getValue(Cesium.JulianDate.now());
    return gltfCenter;
  }
  returnGltfCentorDegree(gltf: any) {
  ...
  editRtation() {
    ...
    this.createCircle(option.originDegree.x, option.originDegree.y, option.originDegree.z, option.length);
  }
  createCircle(x: any, y: any, height: any, radius: any) {
    const position = [];
    ...
    const matrix = Cesium.Transforms.eastNorthUpToFixedFrame(new Cesium.Cartesian3.fromDegrees(x, y, height));
    //绕Z轴
    const axisSphereZ = this.createAxisSphere('model_edit_zCircle', position, matrix, Cesium.Color.RED);
    this._viewer.scene.primitives.add(axisSphereZ);

    //绕Y周
    const axisSphereY = this.createAxisSphere('model_edit_yCircle', position, matrix, Cesium.Color.BLUE);
    ...
    //绕X周
    const axisSphereX = this.createAxisSphere('model_edit_xCircle', position, matrix, Cesium.Color.GREEN);
  ...
  createAxisSphere(name: any, position: any, matrix: any, color: any) {
  ...
  updateLineArrow(gltf: any, originDegree: any) {
  ...
  initEvent() {
    ...
    $this._handler.setInputAction(function (event: any) {
      ...
      console.log(pick);
      if (pick && pick.primitive && pick.primitive._name && pick.primitive._name.indexOf('model_edit') != -1) {
        viewer.scene.screenSpaceCameraController.enableRotate = false; //锁定相机
        //高亮加粗显示
        $this._currentPick = pick.primitive;
        $this._currentPick.width = 25;
        let downPos = viewer.scene.camera.pickEllipsoid(event.position, viewer.scene.globe.ellipsoid);
        ...
        if (downPos && Cesium.defined(downPos)) {
          ...
          $this._handler.setInputAction(function (movement: any) {
            let endPos = viewer.scene.camera.pickEllipsoid(movement.endPosition, viewer.scene.globe.ellipsoid);
  ...
  //更新模型位置
  updateModel(params: any, _tx: any, _ty: any, _tz: any, _rx: any, _ry: any, _rz: any) {
    if (this._coordArrows) {
      let cartesian = new Cesium.Cartesian3.fromDegrees(params.tx + _tx, params.ty + _ty, params.tz + _tz);
    ...
    if (this._coordCircle) {
      ...
      let hpr = new Cesium.HeadingPitchRoll(Cesium.Math.toRadians(rx + _rx), Cesium.Math.toRadians(ry + _ry), Cesium.Math.toRadians(rz + _rz));
      let orientation = Cesium.Transforms.headingPitchRollQuaternion(this._gltf.position.getValue(Cesium.JulianDate.now()), hpr);
      this._gltf.orientation = orientation;
    }
  }
  //绘制箭头
  initLineArrow(originDegree: any, targetDegree: any) {
    console.log(originDegree);
    const arrows = new Cesium.PolylineCollection();
    const xPos = [originDegree.x, originDegree.y, originDegree.z, targetDegree.x, originDegree.y, originDegree.z];
    this.darwArrow(arrows, 'model_edit_xArrow', xPos, Cesium.Color.GREEN);
    const yPos = [originDegree.x, originDegree.y, originDegree.z, originDegree.x, targetDegree.y, originDegree.z];
    this.darwArrow(arrows, 'model_edit_yArrow', yPos, Cesium.Color.BLUE);
    const zPos = [originDegree.x, originDegree.y, originDegree.z, originDegree.x, originDegree.y, targetDegree.z];
  ...
  darwArrow(arrows: any, name: any, positions: any, color: any) {
